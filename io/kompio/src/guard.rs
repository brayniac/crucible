...
/// Inline type-erased storage for a `SendGuard` (64 bytes max).
/// Avoids heap allocation for typical guards (~48 bytes).
pub struct GuardBox {
    storage: [MaybeUninit<u8>; 64],
...
impl GuardBox {
    ...
    pub fn new<G: SendGuard>(guard: G) -> Self {
        ...
        let mut storage: [MaybeUninit<u8>; 64] = [MaybeUninit::uninit(); 64];

        // Safety: We checked size and alignment above. storage is 64 bytes
        // with alignment >= 8 (it's in a struct that will be aligned).
        unsafe {
