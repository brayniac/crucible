//! Integration tests: QUIC echo server using krio's EventHandler + QuicEndpoint.
//!
//! Each test launches a krio server with UDP, connects a QUIC client (driven
...
impl QuicEchoServer {
    /// Process events and flush sends. Takes quic out of Option to avoid
    /// borrow conflicts between quic and read_buf.
        ...
        while let Some(event) = quic.poll_event() {
            match event {
                QuicEvent::StreamReadable { conn, stream } => loop {
                    ...
                    if n > 0 {
                        let _ = quic.stream_send(conn, stream, &self.read_buf[..n]);
                    }
                    if fin {
...
impl EventHandler for QuicEchoServer {
    ...
    fn create_for_worker(_worker_id: usize) -> Self {
        ...
        // The actual address comes from the bound UDP socket.
