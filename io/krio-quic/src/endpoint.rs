...
/// A sans-IO QUIC endpoint that integrates with krio's event loop.
///
/// Wraps [`quinn_proto::Endpoint`] and exposes an event-queue API.
/// Each krio worker should own its own `QuicEndpoint`.
///
/// # Usage
///
/// 1. Feed incoming UDP datagrams via [`handle_datagram`](Self::handle_datagram).
/// 2. Drive connection timers via [`drive_timers`](Self::drive_timers) from `on_tick`.
/// 3. Poll application events via [`poll_event`](Self::poll_event).
/// 4. Flush outgoing packets via [`flush_sends`](Self::flush_sends).
...
impl QuicEndpoint {
    ...
    /// Drain the send queue by sending packets via `ctx.send_to()`.
    ///
    /// Returns the number of packets sent. Stops when the queue is empty or
    /// when `send_to` fails (typically because a send is already in-flight).
    pub fn flush_sends(&mut self, ctx: &mut DriverCtx, socket: UdpToken) -> usize {
        if let Some(pkt) = self.send_queue.front()
            && ctx.send_to(socket, pkt.destination, &pkt.data).is_ok()
        {
            self.send_queue.pop_front();
            // krio allows only one UDP send in-flight per socket.
            return 1;
        }
        0
    }
            ...
            match event {
                ...
                Event::Stream(stream_event) => match stream_event {
                    ...
                    StreamEvent::Stopped { .. } | StreamEvent::Available { .. } => {
                        // Not surfaced to application in Phase 1.
                    }
                },
