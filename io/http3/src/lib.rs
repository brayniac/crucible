//! HTTP/3 framing layer for krio-quic.
//!
//! This crate provides a sans-IO, completion-based HTTP/3 framing layer that
//! sits on top of [`krio_quic::QuicEndpoint`]. It handles:
//!
...
//!        │
//!   ┌────▼────┐
//!   │krio-quic│  QUIC transport (quinn-proto wrapper)
//!   │QuicEndp.│  QuicEvent: StreamReadable, StreamOpened, etc.
//!   └────┬────┘
//!        │ stream_recv() / stream_send()
//!   ┌────▼────┐
//!   │  http3  │  HTTP/3 framing + QPACK
//!   │H3Conn   │  H3Event: Request, Data, etc.
//!   └─────────┘
//! ```
//!
//! # Example
...
//!
//! // In EventHandler, after quic.poll_event():
