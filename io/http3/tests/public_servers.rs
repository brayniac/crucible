...
//!
//!   cargo test -p http3 --test public_servers -- --ignored --nocapture
...
impl QuicClient {
    ...
    fn send_h3_settings(&mut self) {
        ...
        let mut buf = Vec::new();
        http3::frame::encode_varint(&mut buf, 0x00); // control stream type
        http3::Frame::Settings(http3::Settings::default()).encode(&mut buf);
    ...
    fn send_get(&mut self, authority: &str, path: &str) -> quinn_proto::StreamId {
        ...
        let headers = vec![
            http3::HeaderField::new(b":method", b"GET"),
            http3::HeaderField::new(b":path", path.as_bytes()),
            http3::HeaderField::new(b":scheme", b"https"),
            http3::HeaderField::new(b":authority", authority.as_bytes()),
            http3::HeaderField::new(b"user-agent", b"crucible-http3/0.1"),
        ];
    ...
    fn recv_response(
        ...
        while offset < raw.len() {
            match http3::frame::decode_frame(&raw[offset..]) {
