//! Integration tests: HTTP/3 server using krio + krio-quic + http3.
//!
//! Each test launches a krio server with an H3 EventHandler, connects a
...
struct H3Server {
    ...
    h3: H3Connection,
    socket: Option<UdpToken>,
}

impl H3Server {
    fn drive(&mut self, ctx: &mut DriverCtx) {
            ...
            match event {
                H3Event::Request {
                    ...
                    if is_get || end_stream {
                        ...
                        let _ = self.h3.send_data(&mut quic, stream_id, b"hello", true);
                    }
                    // For POST with body, wait for Data event.
                }
                H3Event::Data {
                    stream_id,
                    data,
                    ...
                    if end_stream {
                        ...
                        let _ =
                            self.h3
                                .send_response(&mut quic, stream_id, &response_headers, false);
                        let _ = self.h3.send_data(&mut quic, stream_id, &data, true);
                    }
                }
...
impl EventHandler for H3Server {
    ...
    fn on_datagram(
        ...
        self.quic
            .as_mut()
            .unwrap()
            .handle_datagram(Instant::now(), data, peer);
        self.drive(ctx);
    }

    fn on_tick(&mut self, ctx: &mut DriverCtx) {
        self.quic.as_mut().unwrap().drive_timers(Instant::now());
        self.drive(ctx);
    }
    ...
    fn create_for_worker(_worker_id: usize) -> Self {
        let config = SERVER_CONFIG
            ...
            .unwrap()
            .take()
            .expect("no QUIC server config");

        let local_addr: SocketAddr = "0.0.0.0:0".parse().unwrap();

        H3Server {
            quic: Some(QuicEndpoint::new(config, local_addr)),
            h3: H3Connection::new(Settings::default()),
            socket: None,
        }
    }
}
...
impl H3TestClient {
    ...
    fn send_request(
        ...
        http3::qpack::encode(headers, &mut encoded_headers);
        http3::Frame::Headers {
    ...
    fn recv_response(&mut self, stream: quinn_proto::StreamId) -> (Vec<HeaderField>, Vec<u8>) {
        ...
        while offset < raw.len() {
            match http3::frame::decode_frame(&raw[offset..]).unwrap() {
                Some((frame, consumed)) => {
                    offset += consumed;
                    match frame {
                        http3::Frame::Headers { encoded } => {
                            headers = http3::qpack::decode(&encoded).unwrap();
                        }
                        http3::Frame::Data { payload } => {
