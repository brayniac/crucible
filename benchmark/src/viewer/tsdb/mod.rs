...
impl Tsdb {
    pub fn load(path: &Path) -> Result<Self, Box<dyn Error>> {
        ...
        data.source = match metadata.get("source").map(|v: &String| v.as_str()) {
            Some("rezolus") => "Rezolus".to_string(),
            Some("crucible-benchmark") => "Crucible Benchmark".to_string(),
            _ => "unknown".to_string(),
        };

        data.version = match metadata.get("version").map(|v: &String| v.as_str()) {
            Some(s) => s.to_string(),
            _ => "unknown".to_string(),
        };
        ...
        for batch in reader.into_iter().flatten() {
            ...
            for (id, field) in schema.fields().iter().enumerate() {
                ...
                let name = if let Some(n) = meta.get("metric").cloned() {
                    ...
                    // Try field name if no metric metadata
                    debug!(
                        "Field '{}' has no 'metric' metadata, using field name",
                        field.name()
                    );
                ...
                let grouping_power: Option<Result<u8, ParseIntError>> =
                    meta.remove("grouping_power").map(|v: String| v.parse());

                let max_value_power: Option<Result<u8, ParseIntError>> =
                    meta.remove("max_value_power").map(|v: String| v.parse());
                ...
                // Determine if this metric is a gauge based on name patterns
                let is_gauge = name.contains("_active")
                    || name.contains("_current")
                    || name.contains("_size")
                ...
                match column.data_type() {
                    DataType::UInt64 => {
                        ...
                        if is_gauge {
                            // Treat as gauge (point-in-time value)
                            let gauges = data.gauges.entry(name.to_string()).or_default();
                            ...
                            // Treat as counter (cumulative value)
        ...
        // Debug: print what metrics were loaded
        debug!(
            "Loaded {} counter metrics: {:?}",
            data.counters.len(),
            data.counters.keys().collect::<Vec<_>>()
        );
        debug!(
            "Loaded {} gauge metrics: {:?}",
            data.gauges.len(),
            data.gauges.keys().collect::<Vec<_>>()
        );
        debug!(
            "Loaded {} histogram metrics: {:?}",
            data.histograms.len(),
            data.histograms.keys().collect::<Vec<_>>()
        );
    ...
    pub fn histograms(&self, name: &str, labels: impl Into<Labels>) -> Option<HistogramCollection> {
        ...
        if let Some(histograms) = self.histograms.get(name) {
            let histograms = histograms.filter(&labels);
            if !histograms.is_empty() {
                return Some(histograms);
            }
        }

        // Try with :buckets suffix (common naming convention)
        let buckets_name = format!("{}:buckets", name);
        if let Some(histograms) = self.histograms.get(&buckets_name) {
            let histograms = histograms.filter(&labels);
            if !histograms.is_empty() {
                return Some(histograms);
            }
