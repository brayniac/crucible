...
/// Per-worker handle used by the HttpClient to send commands and wake workers.
///
...
pub(crate) struct WorkerHandle {
    tx: crossbeam_channel::Sender<HttpCommand>,
    eventfd: RawFd,
...
impl WorkerHandle {
    ...
    pub fn send(&self, cmd: HttpCommand) -> Result<(), HttpError> {
        self.tx.send(cmd).map_err(|_| HttpError::WorkerClosed)?;
