...
/// Per-worker handle used by MomentoClient to send commands and wake workers.
///
...
pub(crate) struct WorkerHandle {
    tx: crossbeam_channel::Sender<MomentoCommand>,
    eventfd: RawFd,
...
impl WorkerHandle {
    ...
    pub fn send(&self, cmd: MomentoCommand) -> Result<(), MomentoError> {
        self.tx.send(cmd).map_err(|_| MomentoError::WorkerClosed)?;
