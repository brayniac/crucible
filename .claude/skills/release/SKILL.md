...
3. **Determine the new version**:
   ```bash
   # Get current version from server/Cargo.toml
   CURRENT=$(grep '^version = ' server/Cargo.toml | head -1 | sed 's/version = "\(.*\)"/\1/')
...
3. The `release.yml` workflow then builds artifacts
