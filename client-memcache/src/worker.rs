...
/// ClientHandler implements EventHandler for the memcache client crate.
...
impl ClientHandler {
    ...
    fn send_command(&mut self, ctx: &mut DriverCtx, cmd: Command, conn_token: ConnToken) {
        ...
        if is_large_set {
            ...
            // Encode memcache framing around the value (not the value itself)
            self.header_buf.clear();
            encode_set_header(&mut self.header_buf, &key, value.len(), flags, exptime);

            self.suffix_buf.clear();
            encode_set_suffix(&mut self.suffix_buf);
            ...
            // Copy path for small commands (GET, DEL, VERSION, small SET)
...
impl EventHandler for ClientHandler {
    fn create_for_worker(_worker_id: usize) -> Self {
        ...
        ClientHandler {
            ...
            suffix_buf: Vec::with_capacity(4),
    ...
    fn on_data(&mut self, _ctx: &mut DriverCtx, conn: ConnToken, data: &[u8]) -> usize {
        ...
        let mut consumed = 0;
        loop {
            match protocol_memcache::Response::parse(&data[consumed..]) {
                Ok((response, n)) => {
                    consumed += n;
                    if let Some(pending) = info.in_flight.pop_front() {
                        ...
                        match &pending.kind {
                            ...
                            ResponseKind::Version(_) => {}
                        }
                        pending.complete(response);
                    }
                }
                Err(protocol_memcache::ParseError::Incomplete) => break,
                Err(e) => {
