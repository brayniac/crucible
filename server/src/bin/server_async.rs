//! Crucible async cache server binary.
//!
//! Identical to `crucible-server` but uses krio's AsyncEventHandler
//! (one async task per connection) instead of the callback EventHandler.
//! Intended for A/B benchmarking against the callback server.
...
#[derive(Parser)]
#[command(name = "crucible-server-async")]
#[command(about = "High-performance cache server (async runtime)")]
struct Args {
...
fn run_with_cache<C: cache_core::Cache + 'static>(
    ...
    server::async_native::run(&config, cache, shutdown, drain_timeout)
}
